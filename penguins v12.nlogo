...
This model tries to simulate the heat preservation technique used by the penguin emperor extreme cold weather.

## HOW IT WORKS

Each iteration procedure is made of several steps.

Step 1)	Identify each penguin if it’s part of a huddle or not. A penguin can be classified part of a huddle if the number of neighbouring penguins within a radius R exceed some threshold "death-warning-limit." Also, 

Step 2)	The penguins outside the huddle make a number of steps towards joining the huddle. Each one, rotate around the huddle and join from the back.

Step 3)	The penguins inside the huddle make a movement. The penguins are selected in the order of an axis position (e.g. the reverse Y axis) so that each penguin has an opening in front of it to move before it makes a step.

Step 4)	All the penguins gain some amount heat according to a variable to simulate body heat generation.

Step 5)	All penguins lose some heat according to a variable ("body-heat-lost-per-tick") to simulate heat loss to the environment. If a penguins is on the perimeter (i.e. exposed to the weather) lose temperature significantly more that a penguin within the huddle. The lost heat gets added to temperature of the surrounding environment.

Step 6)	To simulate heat diffusion, each spot of the environment averages its temperature with the surrounding environment within a certain radius. Furthermore, to simulate wind flowing, the spots of the environment that lies outside the huddle, loses the temperature completely (i.e. reset their temperature to set environment temperature).

Step 7)	Check all penguins if their temperature becomes lower than a certain threshold, they become vulnerable. If a penguins becomes vulnerable for more that a certain number of consecutive steps, it dies.


## HOW TO USE IT


Click the "reset-all" button. Then create penguins by clicking on one of the three buttons within the section "Create Penguins," e.g. "create clumped penguins." Then click on the "go" button (the forever "go").


## THINGS TO NOTICE


The penguinsin the huddle makes a reshuffling movements to so that the penguins inside trade places with those on perimeter.

Explore the case when the penguin do not form a huddle by enabling the switch "no-huddle?." Then compare performance agianst the huddle strategy.

Explore creating "selfish" penguins using the button "create disperse selfish penguins", and compare their performance against regular penguins. You'll notice that the regular perform better with high "death-warning-limit" (e.g. 400 and above). You'll need to wait for number of ticks that is several time folds the number of "death-warning-limit."


## THINGS TO TRY


Try creating mixed number of regular and "selfish" penguins.


## EXTENDING THE MODEL


Finite energy stored: Since the penguins gather and store food as fat in their body then fast for months, it is worth exploring how each strategy perform when the penguins have a finite energy stored.

Reproduction rate: If the number of penguins that die is very small compared to the reproduction rate, then that doesn’t affect the group as much. A further extension to this model might explore the acceptable percentage of mortality without going extinct.

Genetic Algorithm: This model has settings that control many variables such as the body heat generated per tick, body heat lost per tick, the killing body temperature, the heat lost per tick, etc.  Although in this model we used the same settings for the different strategies for fairness, there are might be an optimal settings that could be found using a genetic algorithm.


## CREDITS AND REFERENCES


https://github.com/mshj/Emperor-Penguins-Simulation
@#$#@#$#@
default
true
0
